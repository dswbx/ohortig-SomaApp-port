   ...
   const handleSignIn = async () => {
      try {
         const response = await fetch(
            "https://soma.bknd.run/api/auth/password/login",
            {
               method: "POST",
               headers: {
   ...
   return (
      <SafeAreaView style={styles.mainContainer}>
         ...
         <View style={{ paddingHorizontal: 20, paddingVertical: 40 }}>
            <View style={{ marginBottom: 20 }}>
               ...
               <Pressable onPress={() => alert("Feature coming soon")}>
                  <Text style={[styles.linkText, { marginVertical: 20 }]}>
                     Forgot Password?
                  </Text>
               </Pressable>
            </View>
            ...
            {error ? (
               <Text style={[styles.bodyText, { color: "red", marginTop: 10 }]}>
                  {error}
               </Text>
            ) : null}
            <SignUpGoogleButton />
         ...
         <View style={styles.bottomContainer}>
            <Text style={{ marginHorizontal: 2.5 }}>
               Don't have an account?
            </Text>
            <Pressable onPress={() => alert("Feature coming soon")}>
               <Text style={[styles.linkText, { marginHorizontal: 2.5 }]}>
                  Sign Up
               </Text>
            </Pressable>
         </View>
